...
class PrivacyVerifier:
    ...
    def compute_membership_inference_resistance(
        ...
        """
        Estimate resistance to individual client identification.
        
        Measures how difficult it is to determine if a specific client
        contributed to the aggregated model by analyzing weight similarity.
    ...
    def _compute_weight_similarity(
        ...
        for layer_name in weights1:
            if layer_name in weights2:
                # Reconstruct weight matrices: W = A @ B^T
                W1 = torch.matmul(
                    weights1[layer_name]['A'],
                    weights1[layer_name]['B'].T
                )
                W2 = torch.matmul(
                    weights2[layer_name]['A'],
                    weights2[layer_name]['B'].T
                )
                ...
                if w1_flat.shape == w2_flat.shape:
