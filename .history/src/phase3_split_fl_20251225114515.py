...
class LoRAAdapter(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Get the actual hidden dimension from input
        hidden_dim = x.shape[-1]
        
        # Handle dimension mismatch (from dummy models)
        if hidden_dim != self.in_dim:
            # Project to correct dimension first
            x_projected = torch.nn.functional.linear(
                x, 
                torch.eye(self.in_dim, hidden_dim, device=x.device, dtype=x.dtype)
            )
        else:
            x_projected = x
        
        # x @ A @ B^T
        result = torch.matmul(torch.matmul(x_projected, self.A), self.B.T)
        ...
        if hidden_dim != self.in_dim:
            result = torch.nn.functional.linear(
                result,
                torch.eye(hidden_dim, self.in_dim, device=result.device, dtype=result.dtype)
            )
