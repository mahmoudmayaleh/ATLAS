...
class TestGradientExtractor(unittest.TestCase):
    ...
    def test_extract_single(self):
        ...
        # Extract fingerprint using SAME gradient shape as training
        test_grads = {f'layer{i}': torch.randn(10, 20) for i in range(3)}
        fingerprint = self.extractor.extract(test_grads)
    ...
    def test_reproducibility(self):
        ...
        # Use SAME gradient shape as training for extraction
        test_grads = {f'layer{i}': torch.randn(10, 20) for i in range(3)}
        
        # Extract twice
        fp1 = self.extractor.extract(test_grads)
        fp2 = self.extractor.extract(test_grads)
...
class TestTaskClusterer(unittest.TestCase):
    ...
    def test_cluster_synthetic_data(self):
        ...
        # Silhouette score should be reasonable (>0 means better than random)
        self.assertGreater(result['silhouette_score'], 0.2)
